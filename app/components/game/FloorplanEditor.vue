...
import type { MapAsset, PlayerSpawn } from '#shared/game/types';
...
<template>
  <div v-if="props.open" class="editor-backdrop" @click.self="emit('close')" @pointerup="endDrag">
    <section class="editor-modal" role="dialog" aria-modal="true" aria-labelledby="editor-title">
      ...
      <div class="spawn-controls"><span class="section-label">Player start position</span><label for="spawn-x">X <input id="spawn-x" v-model.number="playerSpawn.x" type="number" min="-10" max="10" step=".5"></label><label for="spawn-z">Z <input id="spawn-z" v-model.number="playerSpawn.z" type="number" min="-8" max="8" step=".5"></label><span class="spawn-help">Players appear here in a new game.</span></div>
      <footer class="editor-footer"><span class="asset-status">{{ assets.length }} components in floorplan</span><button type="button" class="deploy-button" @click="emit('deploy', assets, { ...playerSpawn })">Deploy floorplan ↗</button></footer>
    </section>
  </div>
</template>
...
.spawn-controls { display: flex; align-items: center; gap: .7rem; padding: .55rem .8rem; border-top: 1px solid #1e293b; background: rgba(2,6,23,.35); }.spawn-controls label { color: #bfdbfe; font: .65rem 'Courier New', monospace; text-transform: uppercase; }.spawn-controls input { width: 4rem; margin-left: .25rem; border: 1px solid #475569; padding: .3rem; background: #0f172a; color: #f8fafc; font: .7rem 'Courier New', monospace; }.spawn-help { color: #64748b; font: .62rem 'Courier New', monospace; }.player-spawn { position: absolute; z-index: 3; width: 2.2rem; height: 2.2rem; padding: 0; border: 3px solid #fff7b2; border-radius: 50%; background: #ffcc00; box-shadow: 0 0 0 2px #7c5f00, 0 0 18px rgba(255,204,0,.75); color: #0f172a; cursor: grab; font: 700 .45rem 'Courier New', monospace; transform: translate(-50%, -50%); touch-action: none; }.player-spawn.dragging { cursor: grabbing; box-shadow: 0 0 0 3px #fff7b2, 0 0 22px rgba(255,204,0,.95); }
</style>
